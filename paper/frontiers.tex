...
\section{}
MVPA-Light is a MATLAB toolbox for multivariate pattern analysis (MVPA). It provides native implementations of a range of classifiers (LDA, Logistic Regression, SVM, kernel FDA, Naive Bayes, and ensemble methods) and regression models (ridge, kernel ridge), using modern optimization algorithms. High-level functions allow for the multivariate analysis of multi-dimensional data, including generalization (e.g. time x time) and searchlight analysis. The toolbox performs  cross-validation, hyperparameter tuning, and nested preprocessing and computes various classification and regression metrics. It is modular, easily extendable, and is shipped with sample data and example scripts. Furthermore, it offers interfaces for LIBSVM and LIBLINEAR as well as an integration into the FieldTrip neuroimaging toolbox.
...
A standard Desktop or Laptop is sufficient to run MVPA-Light. The memory requirement is mostly dictated by the size of the dataset. Some functions operate on a copy of the data. It is therefore recommended that the available RAM exceeds the size of the dataset by at least a factor of two (e.g. 4+ GB RAM for a 2 GB dataset). MVPA-Light is supported by MATLAB 2012a and more recent versions. The Statistics toolbox is required at some points in the toolbox (e.g. for calculating t-values). The cluster permutation test in \ttt{mv\_statistics} uses the Image Processing toolbox to extract the clusters. No other toolboxes are required. 
...
\red{For regression tasks, there is a single function \ttt{mv\_regress} that is the equivalent of \ttt{mv\_classify} for regression. That is, it is a general-purpose that works with data of arbitrary dimensionality and supports both searchlight and generalization. Regression covers both univariate (vector-valued responses) and multivariate (matrix-valued responses) regression tasks.}
...
In most cases, the quantity of interest is not the raw classifier output but rather a performance metric that is calculated from the classifier outputs. It should reflect the degree of separability between the classes. The desired metric can be specified by e.g. setting \ttt{cfg.metric = 'accuracy'}. Multiple metrics can be requested by providing a cell array, e.g. \ttt{cfg.metric = \{'accuracy', 'auc'\}}. \prettyref{tab:metrics} lists the metrics implemented in MVPA-Light. For a thorough discussion of classification metrics, refer to  \cite{Sokolova2009ATasks}.
...
MVPA is performed at the sensor level using a LDA classifier. All analyses are cross-validated using 5-fold or 10-fold cross-validation. Only the MEG channels are used as features except for analysis 2, where different sets of channels are compared. The statistical significance for some of these results was assessed as follows

\begin{itemize}
    \item Level 1 analysis (first subject only) based on the time generalization results for subject 1 contrasting famous vs scrambled faces using the EEG channels only. Classification accuracy is used as a metric. Three statistical tests are compared: binomial, permutation, and cluster permutation tests. Both permutation tests are based on 500 permutations of the class labels. Only the cluster permutation test has been corrected for multiple comparisons by using a cluster statistic. A cut-off value of 0.6 for accuracy is chosen for the cluster statistic. 
    \item Level 2 analysis (across subjects). 
    
\end{itemize}

All scripts used for the analysis are available in the accompanying GitHub repository \footnote{https://github.com/treder/MVPA-Light-Paper}.
...
\red{\textit{Level 1 statistical analysis}. Images depict the AUC values masked by whether or not a time-time point is significant according to each of the three tests. Both uncorrected tests (binomial and permutation test) exhibit spurious effects even at pre-stimulus time. The cluster permutation test seems to robustly identify a contiguous cluster that discriminates between the two classes.}

\red{\textit{Level 2 statistical analysis}. Group statistical analysis of the time-frequency classification data. A within-subjects design was used contrasting the AUC against the chance level of 0.5. The }
...
The goal of this section is to develop a dual regularization approach for LDA that is equivalent to the primal regularization approach. This is important in order to assure that switching from primal to dual form with an equal regularization magnitude $\lambda$ does not affect the direction the solution $\w$. The approach consists of two steps. First, we need to calculate the Ledoit-Wolf estimate for the optimal regularization hyperparameter $\lambda^*$ using the Gram matrix instead of the covariance matrix. Second, we need to regularize the dual of LDA such that the primal and dual solutions yields the same $\w$.
...
The goal is to rewrite \prettyref{eq:LedoitWolf} in terms of the Gram matrix $\K= \X\X^\top$. Towards this end, note that in the numerator can be rewritten as $||\S - \x_i\x_i^\top||_F^2 = \Tr{\K^2}/n^2 - 2\,\Tr{\S\,\x_i\x_i^\top} + \langle\x_i,\x_i\rangle^2$. For the middle term we have $\Tr{\S\,\x_i\x_i^\top} = \Tr{\frac{1}{n}\sum_{j=1}^n \x_j\x_i^\top\langle\x_i,\x_j\rangle} = \frac{1}{n}\sum_{j=1}^n \langle\x_i,\x_j\rangle^2$. In other words, we can rewrite the Ledoit-Wolf estimate in terms of inner products as 
...
where $\K$ is the Gram matrix as defined above. The optimal $\al$ is hence given by $(\mathbf{N} + \lambda\K)^{-1}(\M_1 - \M_2)$.
...
I would like to thank colleagues from the Psychology department at University of Birmingham (particularly the groups of Bernhard Staresina, Maria Wimber, Simon Hanslmayr, and Ian Charest) for advice and early adaptation of the toolbox, and Jan-Mathijs Schoffelen and Sophie Arana for their efforts towards integrating it into FieldTrip. Many thanks to all contributors to the GitHub repository
...
\caption{Results for the classification analysis of the \cite{Wakeman2015ADataset} data.}\label{fig:results}
\end{figure}
