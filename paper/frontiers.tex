...
\red{Multivariate pattern analysis (MVPA) refers to a set of tools . It draws heavily on \textit{supervised learning}, a branch of machine learning dealing with classification and regression problems. In addition to these, bespoke tools such as Representational Similarity Analysis \citep{Kriegeskorte2008} have been developed by the neuroimaging community. In the fMRI literature, MVPA was first popularized by the seminal work by Haxby et al. \citep{Haxby2001,Haxby2012,Norman2006}. \cite{Farwell}}
...
\subsection{\red{Running example}}


\subsection{\red{MVPA glossary}}

MVPA comes with its own set of commonly used terms and concepts such as 'hyperparameter' and 'cross-validation'. Most of these originate from the field of \textit{supervised learning}, the branch of machine learning that MVPA is based on. Since they are used extensively throughout the paper, a list of terms is provided here in alphabetical order. For a more comprehensive review of these concepts, the reader is referred to the numerous MVPA review paper that have been published over the years   \citep{Mur2009,Misaki2010ComparisonFMRI,Grootswagers2017DecodingData,Pereira2009,Varoquaux2017}. Furthermore, for an in-depth introduction into machine learning refer to the standard textbooks such as \cite{James2013,Hastie2009,Bishop2007}. 


To provide examples, ... we will refer to the following hypothetical experiment: Subjects are performing 

throughout the paper ... we will refer ... 

hypothetical visual experiment ... In each trial, subjects are presented an imag
...
\begin{itemize}
\item \textit{Binary classifier}. A classifier trained on data that contains two classes, e.g. ... If there is more than two classes, the classifier is a multi-class classifier.
\item \textit{Classification}. One of the primary applications of MVPA. In classification, a classifier takes a multivariate pattern of brain activity (referred to as \textit{feature vector}) as input and maps it onto a categorical brain state or experimental condition (referred to as \textit{class label}). In other words, the task of a classifier is to predict class labels given features. 

"Does brain activity differentiate between faces and houses?" is usually less interesting than the question "

In an fMRI experiment, ... "Does "

In classification we seek to answer questions such as the following: 

It is worth noting that classification is often performed at the level of individual participants, taking trials as data samples. The classifier output then corresponds to predictions for single trials. 

... In classification, we go the opposite route ... Often, the \textit{if} is not enough ... 
\item \textit{Classifier}. An algorithm that performs classification. It takes a feature vector as input and produces a class label as output. E.g. Linear Discriminant Analysis (LDA) and Support Vector Machine (SVM) are two popular cla
\item \textit{Class label}. Categorical variable that attaches a label to each sample or trial. In classification, the class label acts as the dependent variable. E.g. in a visual experiment wherein subjects view a face or a house in each trial, the class labels would be 'face' and 'house'. Class labels are often encoded by numbers, e.g. 'face' = 1 and 'house' = 2.
...
\item \textit{Decoder}. An alternative term for a \textit{classifier} or \textit{regression model}. This term is popular in the neuroimaging literature, especially if the input is a pattern of brain activity.
\item \textit{Feature}. Also known as independent variable or predictor. A feature serves as an input to a classifier or regression model. If the data is tabular with rows representing samples, it typically corresponds to one of the columns. E.g. grey-matter density for a given voxel, or EEG voltage at a given time point.
\item \textit{Feature vector}. For each sample, features are stored in a vector. E.g. imagine a EEG measurement with electrodes Fz, Cz, and Oz and corresponding voltages 40 $\mu$V, 65 $\mu$V, and 97 $\mu$V. The voltage at each EEG sensor represents a feature, so the corresponding feature vector is the vector [40, 65, 97]$\in\R^3$.
...
\item \textit{Response}. The equivalent to a class label in regression. Unlike class labels, a response is typically a continuous variable. E.g. in a reaction time task, the reaction time measured in each trial can serve as a response.
\item \textit{Testing}. 
