...
In multi-dimensional neuroimaging data, a model needs to be trained on a single subject's data potentially thousands of times. This is exacerbated in permutation testing, when this process needs to be repeated hundreds or thousands of times. A key variable to computational efficiency is training speed, that is, the amount of time needed to optimize the model parameters on training data. To benchmark the regression models and classifiers of MVPA-Light, they will be compared to other MATLAB implementations as well as classifiers in Python (using Sci-kit learn) and R (using XX). Many of the other MVPA toolboxes mostly rely on external packages such as LIBSVM and LIBLINEAR (this applies to e.g. DDTBOX, ProNTo, TDT), so MVPA-Light models will be compared to ... rather than the actual implementations 

... 
To be as unbiased as possible, only ... without hyperparameter tuning ... 


\begin{enumerate}
    \item Out-of-the-box performance: What is the classification accuracy ... using default parameters ... 
    \item Training speed: : Training time in milliseconds ... To avoid conflation between training time and nested hyperparameter tuning, all hyperparameters will be fixed a priori. ... 
\end{enumerate}

the following three datasets

\begin{itemize}
    \item MEG single-subjects. ... The \cite{Wakeman2015ADataset} ...  Since many tested  classifiers are binary, only the classes famous vs scrambled faces are considered. 
    \item MEG super-subject. A single large dataset is obtained by concatenating all trials of all MEG subjects to a 'super-subject'.  
    \item fMRI: ... 6 subjects ... Voxels with zero signal are removed, yielding a dataset of 216 samples and XXX voxels serving as feature vector. 
\end{itemize}

While the single-subject MEG data is a dataset that of typical size for MEEG analysis, the latter two ... map out extreme cases ... 
The 

Benchmarking is based on the MEEG dataset, performing classification for every time point. For vetting training time ... Two additional datasets are considered to also cover extreme cases. Firstly, MEEG trials of all subjects are combined to a single dataset. This yields a "$N\gg P$" dataset, that is, data where trials vastly outnumber features (16,000 trials, 366 features). Secondly, we use full-brain \cite{Haxby2001}  dataset. By involving all voxels as features a "$P\gg N$" dataset is created, that is, data where features vastly outnumber trials (XXX trials per subject, XXX features). These latter two cases test the efficiency of a classifier dealing with a large number of trials or a large number of features.


\begin{itemize}
    \item MVPA-Light: lda, logreg, svm, libsvm, liblinear, naive bayes, ridge, kernel ridge.
    \item MATLAB: fitdiscm, lassoglm, fitcnb, fitcsvm, ridge, fitcsvr.
...
Scripts used for benchmarking as well as the fMRI and MEEG analyses are available in the accompanying GitHub repository\footnote{https://github.com/treder/MVPA-Light-Paper}.
...
as expected ... kernel classifiers are not strongly affected ... by 
