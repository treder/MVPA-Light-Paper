...
\red{Multivariate pattern analysis (MVPA) refers to a set of multivariate tools for the analysis of brain activity and structure. It draws heavily on \textit{supervised learning}, a branch of machine learning dealing with classification and regression problems. In addition, bespoke tools such as Representational Similarity Analysis \citep{Kriegeskorte2008} are part of the MVPA toolkit. In the fMRI literature, MVPA was first popularized by the seminal work of Haxby et al. \citep{Haxby2001,Haxby2012,Norman2006}. In EEG-based brain-computer interface research, multivariate classification has been used since at least the 1980s \cite{Farwell}, but it did not become a mainstream tool in cognitive EEG/MEG research until the early 2010s \citep{Blankertz2011,Lemm2011,Mur2009,Grootswagers2017DecodingData,Pereira2009,Varoquaux2017}. How does MVPA compare to traditional statistical methods such as t-test and Analysis of Variance (ANOVA)? Traditional methods are usually \textit{univariate} (although there is e.g. a multivariate version of the t-test), that is, a test is usually performed for each dependent variable (e.g. voxel, sensor, time point) separately. Clearly, some information is missed with this approach: mental representations are encoded as distributed patterns (a.k.a. \textit{multivariate patterns}) of brain activity \citep{Haxby2001}. By considering the whole pattern at once, MVPA is perfectly designed to exploit the distributed information.}

To highlight this difference with an example, consider a hypothetical visual experiment: In each trial, subjects are presented an image of either a face or a house and their brain activity is recorded. To make sure they maintain attention, subjects are instructed to indicate via a button press whether the image represents a face or a house. This experiment will be referred to as \textit{'faces vs houses' experiment} throughout this paper.
A t-test applied to fMRI data can be used to answer a question like "Is the activity \textit{at a specific voxel} different for faces vs houses?". In contrast, MVPA addresses the more general question "Is the \textit{pattern of brain activity} different for faces vs houses?". Does this mean that MVPA makes traditional statistical tests superfluous? The answer is a clear 'no'. In fact, MVPA trades in \textit{statistical sensitivity} for \textit{localizability}: A t-test may be insensitive to a true statistical difference because it is blind to correlations across different locations (low sensitivity), but whenever an effect is found the exact location (e.g. voxel coordinate) of the effect is known (perfect localizability). MVPA harnesses relationships between different locations (high sensitivity) but it is more challenging to attribute the effect to a specific location (low localizability). As will be seen in section XX, a MVPA technique called \textit{searchlight analysis} attempts to increase localizability while maintaining sensitivity by using the distributed information within a spatially limited region. Hopefully, this illustrates that MVPA can be considered as a complement, rather than a competitor, to the set of traditional statistical tools.}
It is worth mentioning that there are other ways in which MVPA and traditional statistics differ. For instance, MVPA includes models that can detect non-linear relationships and it employs techniques such as cross-validation that control for overfitting.
...
MVPA comes with its own set of commonly used terms and concepts such as 'hyperparameter' and 'cross-validation', most of which are borrowed from machine learning. Since they are used extensively throughout the paper, a list of terms is provided here in alphabetical order. Understanding these terms can be challenging, so unfamiliar readers are referred to available comprehensive review papers on MVPA  \citep{Mur2009,Misaki2010ComparisonFMRI,Grootswagers2017DecodingData,Pereira2009,Varoquaux2017}. Furthermore, for an in-depth introduction into machine learning refer to the standard textbooks such as \cite{James2013,Hastie2009,Bishop2007}.
...
\item \textit{Classification}. One of the primary applications of MVPA. In classification, a classifier takes a multivariate pattern of brain activity (referred to as \textit{feature vector}) as input and maps it onto a categorical brain state or experimental condition (referred to as \textit{class label}). In the 'faces vs houses' experiment, the classifier is used to investigate whether brain activity patterns can discriminate between faces and houses.
\item \textit{Classifier}. An algorithm that performs classification. It takes a feature vector as input and produces a class label as output. Linear Discriminant Analysis (LDA) and Support Vector Machine (SVM) are two examples of classifiers.
\item \textit{Classifier output}. If a classifier receives the brain activity pattern for a given trials as input, its output is a predicted class label for this trial, e.g. 'face'. It implies that the classifier 'thinks' the brain pattern was generated by a 'face' image. Many classifiers are also able to produce probabilities ("How likely is it that this brain pattern was generated by a 'house' image?") or decision values.
\item \textit{Class label}. Categorical variable that attaches a label to each sample or trial. In classification, the class label acts as the dependent variable. In the 'faces vs houses' experiment, the class labels are 'face' and 'house'. Class labels are often encoded by numbers, e.g. 'face' = 1 and 'house' = 2. Class labels are often arranged as a vector. For instance, the vector [1, 2, 1] would indicate that a subject viewed a face in trial 1, a house in trial 2, and another face in trial 3.
...
\item \textit{Data}. From the perspective of a classifier or regression model, a dataset consists of a number of samples e.g. corresponding to trials in an experiment. Each sample consists of a pair $(\x, y)$ where $\x$ is a feature vector and $y$ is a class label or response.
\item \textit{Decision boundary}. This term refers to the way that classifiers perform the classification task. ... The  ... For linear classifiers, the decision boundary is also known as a hyperplane.
\item \textit{Decision value}. ... For linear classifiers, decision value is the same as the distance to the hyperplane. Decision values are usually unitless.
\item \textit{Decoder}. An alternative term for a \textit{classifier} or \textit{regression model} that is popular in the neuroimaging literature. The term nicely captures the fact that classifier tries to invert the encoding process. During encoding, e.g. a sensory experience such as viewing a face is translated into a pattern of brain activity. During decoding, we start with this pattern of brain activity, and we try to infer whether it was a face or a house that gave rise to the brain pattern.
\item \textit{Feature}. A feature serves as an input to a classifier or regression model. If the data is tabular with rows representing samples, it typically corresponds to one of the columns. E.g. grey-matter density for a given voxel, or EEG voltage at a given time point.
\item \textit{Feature space}. In linear algebra terms, the feature space is the space that contains the feature vectors. The dimensionality of the feature space is equal to the number of features.
...
\item \textit{Hyperparameters}. Hyperparameters are parameters of a classifier or regression model that need to be specified by the user, such as the type and amount of regularization applied, the type of kernel, and the kernel width $\gamma$ for Gaussian kernels. From the user's perspective, hyperparameters can be nuisance parameters: it is sometimes not clear a priori how to set them, but their exact value can have a substantial effect on the result.
\item \textit{Hyperparameter tuning}. If it is unclear ... Train a model multiple times ... using different values for the hyperparameters. ...
\item \textit{Hyperplane}. For linear classifiers, the decision boundary is ... In two dimensions
\item \textit{Loss function}. A function that is used to train a model. During training, one searches for model parameters that minimize the loss function. E.g., in linear regression the sum of squares of the residuals serves as a loss function. The beta coefficients are the parameters that minimize this sum of squares.
...
\item \textit{Overfitting}. Occurs when a model over-adapts to the data ... Overfitting can be spotted when comparing training accuracy with the test accuracy.
\item \textit{Parameters}. Models are governed by parameters, e.g. the beta coefficients in linear regression or the weight vector $\w$ in a linear classifier.
\item \textit{Regression}. One of the primary applications of MVPA (together with classification). Regression is very similar to classification, but we aim to predict a continuous variable rather than a cetegorical class label. ... It shares many characteristics with classification, but it maps features onto a continuous variable (e.g. reaction time) referred to a \textit{responses} rather than class labels. Consequently, predictions of the regression model are continuous  (e.g. predicted reaction time given a brain pattern).
\item \textit{Regression model}. An algorithm that performs regression.
\item \textit{Regularization}. A set of techniques that aim to reduce overfitting. It biases the model towards favouring simple models. Regularization is often directly incorporated into training by adding a penalty term to the loss function. A popular ...
\item \textit{Response}. The equivalent to a class label in regression. Unlike class labels, a response is typically a continuous variable. E.g. in a reaction time task, the reaction time measured in each trial can serve as a response. In the 'faces vs houses' experiment, suppose that reaction time of the button press is also recorded. ...
\item \textit{Testing}.
\item \textit{Test set}. ... Like training data,
\item \textit{Training}. The process of finding the best parameters for a model given  training data.
\item \textit{Training set}. ...
\item \textit{}.
...
The structure of the paper is as follows: ... first the toolbox is introduced ... Then, two datasets ... a MEEG ... and a fMRI dataset are presented. Example analyses on these datasets using MVPA-Light are presented. Finally, a benchmarking analysis is conducted ... to measure the performance ... In this, timing ... classifiers compared to classifiers from other toolboxes in MATLAB, Python, and R.
...
\red{The main workhorses of MVPA are \textit{classifiers} and \textit{regression models}. They are explained in more detail in the glossary. They also form the heart of the MVPA-Light. ... are the algorithms that perform these tasks. They form the heart of a MVPA toolbox ... }


\red{the heart is a collection ... of classifiers and regression models ... A high-level interface comprising a set of high-level functions is built around these models ...  ...  implement cross-validation, hyperparameter tuning, preprocessing, and the calculation of performance metrics.}
...
Cross-validation is implemented in all high-level functions. It is controlled by the following parameters:
...
\prettyref{fig:classifiers} provides a pictorial description of the classifiers. They are implemented using pairs of train/test functions. In the high-level interface, a classifier and its hyperparameters can be specified using \ttt{cfg.model} and \ttt{cfg.hyperparameter}. For instance,
...
with a class-specific mean $\m_i$ and common covariance matrix $\mathbf{\Sigma}$ which have to be  estimated from the training data. \prettyref{eq:bayes} can be evaluated directly to calculate class probabilities. A prediction can be done by selecting the most likely class out of all candidate classes,
