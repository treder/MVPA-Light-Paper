% Performs classification analysis of Wakeman & Henson data [uses FieldTrip
...
Xs = cell(nsubjects, 2);   
cf_cv = cell(nsubjects, 2);
cf_time = cell(nsubjects, 1);
result_time = cell(nsubjects, 1);
% cf_timextime = cell(nsubjects, 1);
cf_freq = cell(nsubjects, 2);
result_freq = cell(nsubjects, 2);
cf_freqxfreq = cell(nsubjects, 1);
cf_meeg = cell(nsubjects, 3);
result_meeg = cell(nsubjects, 3);
clabels = cell(nsubjects, 3); % 1:all 2:famous-vs-scrambled  3: famous-vs-familiar
do_plot = 0;
...
for nn=1:nsubjects     % --- loop across subjects
    ...
    ix_famous_familiar  = (ismember(clabel, [1, 2]));
    ...
    meg_famous_familiar = ft_selectdata(cfg, meg);
    
    clabel_famous_familiar = clabel(ix_famous_familiar);
    ...
    %% Save clabel
    clabels{nn, 1} = clabel;
    clabels{nn, 2} = clabel_famous_scrambled;
    clabels{nn, 3} = clabel_famous_familiar;
    ...
    [cf_time{nn}, result_time{nn}] = mv_classify_timextime(cfg, meg_famous_scrambled.trial, clabel_famous_scrambled);

    if do_plot, mv_plot_result(result, meg_famous_scrambled.time, meg_famous_scrambled.time); end
    
    if nn==1
        % required for level 1 stats
        Xtime = meg_famous_scrambled.trial;
    end
    ...
    [cf_freq{nn,1}, result_freq{nn,1}] = mv_classify(cfg, freq_fs.powspctrm, clabel_famous_scrambled);
    [cf_freq{nn,2}, result_freq{nn,2}] = mv_classify(cfg, freq_ff.powspctrm, clabel_famous_familiar);
    
    if nn==1
        % we need to save the first subjects time-frequency data for the
        % level 1 statistical analysis in statistics_WakemanHenson.m
        Xfreq = freq_fs;
    end
    
    % perf is now a 2-D [frequencies x times] matrix of AUC values
    if do_plot
        mv_plot_result(result_freq{nn,1})
        mv_plot_result(result_freq{nn,2})
%         figure
...
%         title('AUC for famous vs unfamiliar faces at each time-frequency point')
    ...
    [cf_meeg{nn,1}, result_meeg{nn,1}] = mv_classify_across_time(cfg, eeg.trial, clabel_famous_scrambled);
    [cf_meeg{nn,2}, result_meeg{nn,2}] = mv_classify_across_time(cfg, meg.trial, clabel_famous_scrambled);
    [cf_meeg{nn,3}, result_meeg{nn,3}] = mv_classify_across_time(cfg, meeg.trial, clabel_famous_scrambled);
    ...
    if nn==1
        % save first subject separately since we can use it directly for
        % the level 1 statistics
        save([resultsdir 'classification_analysis_subject1'],'nsubjects','clabels', ...
            'erp_time','freq_time', 'Xs', 'cf', 'cf_*', 'result_*','Xfreq','Xtime', 'eeg', 'meg', 'meeg')
    end
end

save([resultsdir 'classification_analysis'],'nsubjects','clabels', ...
    'erp_time','freq_time', 'Xs', 'cf', 'cf_*', 'result_*')
...
av_freq= cat(3, mean(cat(3, cf_freq{:,1}{1}),3), mean(cat(3, cf_freq{:,2}{1}),3) );
se_freq= cat(3, std(cat(3, cf_freq{:,1}{1}),[],3), std(cat(3, cf_freq{:,2}{1}),[],3))/err;
...
%% PLOT
% Generates the plots used in Figure 4 of the paper
